...
class MYUSER(object):
    ...
    @property
    def isAftetDaySell(self):
        return self._isAftetDaySell
    
    @isAftetDaySell.setter
    def isAftetDaySell(self, new_isAftetDaySell):
        self._isAftetDaySell = new_isAftetDaySell
    ...
    def printAllStocksData(self,date):
        if len(self._myStockData)>0:
            for stockData in self._myStockData.items():
                nowprice = float(stockData[1]._nowprice)
                cost = float(stockData[1]._cost)
                p = round(((nowprice - cost) / cost) * 100 , 2)
                msg = (date,' (%s)%s,現價:%s ,成本:%s ,數量:%s ,報酬率:%s ％ '%(stockData[1]._code,stockData[1]._codeNM,nowprice,cost,stockData[1]._quantity,p))
                # print(msg)
                # logging.debug(msg)
                self.addMsg(msg)
